from pl.edu.agh.neural.utils import DataNormalizer
...
class BackpropagationLearning(object):
    ...
    def initialize_previous_weights(self):
        self.previous_weights = [[self.network.get_layer(i).get_neurons()[j].get_weights() for j in range(self.network.get_layer(i).get_neurons())] for i in range(self.network.get_layers())]
    ...
    def calculate_deltas(self,expected_outputs):
        ...
        deltas = [[self.output_delta(neuron,expected_outputs[i]) for i,neuron in enumerate(output_layer.get_neurons())]]
        for i in range(len(self.network.get_layers())-1,-1,-1): #form last layer
            ...
            for j in range(len(layer.get_neurons())):
                ...
                next_layer_weights = [next_layer_neuron.get_weights[j] for next_layer_neuron in self.network.get_layers()[i+1]]
    ...
    def hidden_delta(self,neuron,next_layer_weights,next_layer_deltas):
        ekj = neuron.get_psp_value()
        activator_derivative = neuron.get_activator().derivative(ekj)
        return activator_derivative * sum([next_layer_weights[i] * next_layer_deltas[i]] for i in range(len(next_layer_weights)))
